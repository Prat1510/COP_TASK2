...
void Map::initialise(mode M, ENetPeer* client, bool online) {
	cell MAZE[xsize][ysize];
	long numin = 1;
	initialize(MAZE);   //maze generatiom
	generate(MAZE, numin);
	string MapStringPlayer = savebmp(0, 0, M, MAZE, numin);
	if (online && M==mode::dual){
		ENetPacket* packet = enet_packet_create(&MapStringPlayer[0], strlen(&MapStringPlayer[0])+1, 0);
